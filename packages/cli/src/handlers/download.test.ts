...
`,
    ...
    it('keeps legacy root files in place while writing new slugs under spaces', async () => {
        const tmpDir = await fs.mkdtemp(
        ...
        try {
            ...
            await expect(
                fs.readFile(
                    path.join(tmpDir, 'spaces', 'operations.space.yml'),
                    'utf-8',
                ),
            ).resolves.toContain('slug: operations');
            await expect(
                fs.access(path.join(tmpDir, 'spaces', 'new-finance.space.yml')),
            ).rejects.toMatchObject({ code: 'ENOENT' });
            expect(
